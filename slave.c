...
void calculateMd5(const char *filePath, int writePipe) {
    ...
    printf("%-34s%-34s%-8d\n", filePath, md5Sum, getpid()); // Envía el resultado en el formato especificado
...
int main() {
    ...
    if ((nbytes = read(STDIN_FILENO, inputBuffer, sizeof(inputBuffer))) > 0) {
        ...
        while (token != NULL) {
            calculateMd5(token, STDOUT_FILENO); // Calcula el MD5 y lo envía al STDOUT (pipe)
            token = strtok(NULL, " ");
        }
    }
